...
| `rewrite` | `test_graph_architecture_alias_maps_to_graph_hybrid_0`; `test_global_xattn_pair_strategy_defaults_to_none`; `test_global_xattn_pair_heads_do_not_enable_pair_scoring_without_strategy`; `test_pair_scoring_requires_explicit_diagnostic_strategy_and_cap` | Keep the intent, but assert registry alias policy and executable pair-strategy contracts instead of legacy config mutation. |
...
| `test_graph_architecture_alias_maps_to_graph_hybrid_0` | Preserves `graph` alias behavior | Stage 2 decides alias: migration-only warning or hard unsupported id |
