...
| V2-016 | Engine/legal/history/D6 verification checks semantic identity, row ordering, terminal/current-player state, source/hash/version, and mutation safety. | 01 | `engine/*`, `contracts/*` | Golden, negative, mutation, and semantic identity tests. | planned |
...
| V2-045 | Inference verification catches stale buffers, stale ids, bad masks, wrong rows/shapes, non-finite outputs, and post-validation mutation before policy/search consumption. | 04 | inference transport/adapters | Inference debug bundle, mutation/corruption tests, response validation tests. | planned |
...
| V2-074 | Replay verification preserves trace-to-record-to-projector semantic identity and rejects mutation/corruption before training. | 07 | replay/selfplay/train | Trace-to-record identity tests, D6 replay tests, mutation/corruption report. | planned |
