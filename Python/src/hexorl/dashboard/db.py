...
class DashboardStore:
    ...
    def migrate(self) -> None:
        with sqlite3.connect(self.path) as conn:
            ...
            if existing_tables and "schema_migrations" in existing_tables:
                unsupported = sorted(version for version in current if version < SCHEMA_VERSION)
                if unsupported and SCHEMA_VERSION not in current:
                    raise DashboardSchemaError(
                        f"Unsupported dashboard schema v{unsupported[-1]} at {self.path}; "
                        f"schema v{SCHEMA_VERSION} is required. Rotate or rebuild this dashboard DB."
                    )
