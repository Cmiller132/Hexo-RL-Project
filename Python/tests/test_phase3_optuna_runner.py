...
def test_apply_phase3_params_preserves_architecture_pair_mode_and_heads():
    ...
    assert tuned.selfplay.mcts_simulations == 640
    assert tuned.selfplay.pcr_low_sims == 192
...
def test_phase3_resumed_invocations_do_not_replay_identical_startup_params(tmp_path):
    run_dir = tmp_path / "run"
    promoted_id = "global_xattn_0__none__v1"
    source_dir = run_dir / "candidates" / promoted_id
    source_dir.mkdir(parents=True)
    ...
    append_scorecard(
        ...
        ScorecardRecord(
            ...
            epoch=12,
            completed_epochs=12,
            metadata={"classical_survival_lcb": {"games": 2, "score": 0.42}},
        ),
    )
    base_spec = phase3_study_spec(
        architecture_id="global_xattn_0",
    ...
    con = sqlite3.connect(tmp_path / "phase3.sqlite3")
    try:
        params_by_trial: dict[int, dict[str, float]] = {}
        for trial_id, name, value in con.execute(
            "select trial_id, param_name, param_value from trial_params order by trial_id, param_name"
        ):
            params_by_trial.setdefault(int(trial_id), {})[str(name)] = float(value)
    finally:
        con.close()

    assert len(params_by_trial) == 2
    assert len({json.dumps(params, sort_keys=True) for params in params_by_trial.values()}) == 2


def test_phase3_effective_sampler_seed_is_study_specific_and_resume_specific(tmp_path):
    xattn = phase3_study_spec(
        architecture_id="global_xattn_0",
        pair_mode="none",
        storage=f"sqlite:///{(tmp_path / 'xattn.sqlite3').as_posix()}",
        seed=20260508,
    )
    graph = phase3_study_spec(
        architecture_id="global_graph768_champion",
        pair_mode="none",
        storage=f"sqlite:///{(tmp_path / 'graph.sqlite3').as_posix()}",
        seed=20260508,
    )
    no_seed = phase3_study_spec(
        architecture_id="global_xattn_0",
        pair_mode="none",
        storage=f"sqlite:///{(tmp_path / 'no_seed.sqlite3').as_posix()}",
        seed=None,
    )

    xattn_trial0 = _effective_seed_from_spec(xattn, existing_trials=0)
    graph_trial0 = _effective_seed_from_spec(graph, existing_trials=0)

    assert xattn_trial0 == _effective_seed_from_spec(xattn, existing_trials=0)
    assert xattn_trial0 != graph_trial0
    assert _effective_seed_from_spec(xattn, existing_trials=1) == xattn_trial0 + 1
    assert _effective_seed_from_spec(no_seed, existing_trials=0) is None
