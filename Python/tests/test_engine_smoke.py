...
from hexorl.selfplay.worker import RealMCTSEngine
...
def test_mcts_rejects_shifted_root_offset():
    engine, policy, oq, or_, legal_bytes, root_generation = _initialized_validating_mcts_root()
    with pytest.raises(ValueError, match="offset mismatch"):
        engine.expand_root(policy, 0.0, oq + 1, or_, legal_bytes, root_generation)
...
def test_mcts_rejects_mutated_root_legal_bytes():
    ...
    with pytest.raises(ValueError, match="root legal row mismatch"):
        engine.expand_root(policy, 0.0, oq, or_, rows.tobytes(), root_generation)


def test_mcts_rejects_malformed_root_legal_bytes():
    engine, policy, oq, or_, legal_bytes, root_generation = _initialized_validating_mcts_root()
    with pytest.raises(ValueError, match="legal_bytes length .* multiple of 8"):
        engine.expand_root(policy, 0.0, oq, or_, bytes(legal_bytes) + b"\x00", root_generation)


def test_mcts_root_protocol_exposes_current_four_field_shape():
    engine, _policy, _oq, _or_, _legal_bytes = _initialized_mcts_root()
    assert len(engine.init_root()) == 4


def test_mcts_leaf_protocol_exposes_current_two_field_shape():
    engine, policy, oq, or_, legal_bytes = _initialized_mcts_root()
    engine.expand_root(policy, 0.0, oq, or_, legal_bytes)
    _tensor_batch, count = engine.select_leaves(2)
