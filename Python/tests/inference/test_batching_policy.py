...
def test_batching_policy_keeps_request_kinds_compatible():
    ...
    selected = policy.select_batch(
        [
            ReadyRequest(worker_id=0, count=2, operation_code=1, enqueued_monotonic_s=1.0),
            ReadyRequest(worker_id=1, count=2, operation_code=2, enqueued_monotonic_s=2.0),
            ReadyRequest(worker_id=2, count=1, operation_code=1, enqueued_monotonic_s=3.0),
        ]
    )

    assert selected.worker_ids == [0, 2]
    assert selected.operation_code == 1
...
def test_batching_policy_reports_retryable_backpressure_at_watermark():
    ...
    selected = policy.select_batch(
        [
            ReadyRequest(worker_id=0, count=2, operation_code=1, enqueued_monotonic_s=1.0),
            ReadyRequest(worker_id=1, count=2, operation_code=1, enqueued_monotonic_s=2.0),
        ]
    )
