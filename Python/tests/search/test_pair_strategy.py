...
def test_pair_strategy_none_generates_zero_pair_rows(legal_table):
    ctx = _pair_context(legal_table, strategy=PairStrategy(generation_mode="none", max_pairs=0))
    ev = create_pair_strategy(PAIR_STRATEGY_REGISTRY.build_spec("none", max_pairs=0)).score_root(ctx, _base_eval(ctx))
...
def test_pair_strategy_none_scores_zero_pairs(legal_table):
    ctx = _pair_context(legal_table, strategy=PairStrategy(generation_mode="none", max_pairs=0))
    strategy = create_pair_strategy(PAIR_STRATEGY_REGISTRY.build_spec("none", max_pairs=0))
...
def test_leaf_pair_scoring_requires_explicit_enable_and_cap():
    with pytest.raises(ContractValidationError):
        PAIR_STRATEGY_REGISTRY.validate_spec(PairStrategySpec(name="tactical_only", leaf_enabled=True, max_leaf_pair_rows=0))


def test_full_pair_strategy_requires_diagnostic_root_only_and_cap():
    with pytest.raises(ContractValidationError):
        PAIR_STRATEGY_REGISTRY.validate_spec(PairStrategySpec(name="diagnostic_full_root", root_enabled=True, max_full_pair_rows=10))


def test_capped_pair_strategy_enforces_root_cap(legal_table):
    ctx = _pair_context(legal_table, strategy=PairStrategy(generation_mode="capped_fill", max_pairs=3))
    spec = PAIR_STRATEGY_REGISTRY.build_spec("two_stage_root_only", max_pairs=1)
...
def test_capped_pair_strategy_enforces_leaf_cap(legal_table):
    ctx = _pair_context(legal_table, strategy=PairStrategy(generation_mode="capped_fill", max_pairs=3))
    spec = PAIR_STRATEGY_REGISTRY.build_spec("tactical_only", max_pairs=1)
    ev = create_pair_strategy(spec, pair_scorer=FakePairScorer()).score_root(ctx, _base_eval(ctx))
    assert ev.scored_pair_rows <= 1


def test_diagnostic_full_root_strategy_never_scores_leaves(legal_table):
    ctx = _pair_context(legal_table, strategy=PairStrategy(generation_mode="full_capped", max_pairs=3, allow_full=True))
    spec = PAIR_STRATEGY_REGISTRY.build_spec("diagnostic_full_root", max_pairs=3)
...
def test_explicit_pair_strategy_consumes_pair_scorer_not_pair_targets(legal_table):
    ...
    spec = PAIR_STRATEGY_REGISTRY.build_spec("two_stage_root_only", max_pairs=2)
